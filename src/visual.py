...
def EDA(data_path):
    ...
    for col in data.drop(["ReadingScore", "WritingScore"], axis=1).columns:
...
def classification_result(result):
    for col in set(result.keys()) & set(["LDA", "QDA", "Logistic Regression", "Tree"]):
        data = result[col]
        plot = sns.heatmap(
...
if __name__ == "__main__":
    ...
    my_dm = DM()
    with open("./result4.pickle", "rb") as fr:
